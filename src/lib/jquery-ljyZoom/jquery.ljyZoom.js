/* 
* @Author: Marte
* @Date:   2018-04-03 19:50:48
* @Last Modified by:   Marte
* @Last Modified time: 2018-04-11 21:13:31
*/
;(function($){
    ...
    $.prototype.shift = function(arr){
        let $imgs=$(this).next().find('img');
        for(let i=0;i<arr.length;i++){
            $imgs.eq(i).on('mouseenter',()=>{
                for(let i=0;i<$imgs.length;i++){
                    $imgs.eq(i).css({border:'1px solid #ddd'});
                }
                $imgs.eq(i).css({border:'1px solid #ff9696'});
                $(this).children().attr('src',arr[i]);
            })
        }
    }
    // 系列插件
